...
from methods.pskddpm.models.modules import MLPDiffusion
from methods.pskddpm.models.gaussian_multinomial_distribution import GaussianMultinomialDiffusion
    ...
    def run_loop(self):
        ...
        while step < self.steps:
            ...
            if (step + 1) % self.eval_every == 0:
                self.diffusion.eval()
                x_gen = self.diffusion.sample_all(10000, self.batch_size, ddim=False, steps = 1000)
                x_gen = x_gen.numpy()
                acc = self.dataset.evaluate(x_gen)
